"""
Two-Stage Training: NeRF 64×64 + SR 256×256
...
class InterpCache:
    """插值快取 — FiLM DualProjection 版本：在 256-dim embedding 空間插值。"""

    def __init__(self, cached_hs, specimen_specs, dual_proj, device):
        self.names = []
        ...
        if self.n > 0:
            self.embeddings    = torch.stack(embed_list)   # [N, 256]
            self.label_vecs    = torch.stack(vec_list)     # [N, 7]
            self.mat_feats     = torch.stack(mat_list)     # [N, 7]
            self.hidden_states = torch.stack(hs_list)      # [N, 1024]
    ...
    def sample(self, n_interp, beta_dist, device):
        ...
        i_label = torch.cat([i_vec, i_mat, h_idx, a_idx], dim=-1)  # [n, 16]
...
def main():
    ...
    print(f"[FiLM]   HS({shared_cond_proj.hs_dim}→{shared_cond_proj.hs_out}) "
          f"modulated by Mat({shared_cond_proj.mat_dim}→γ/β[{shared_cond_proj.hs_out}]) "
          f"→ out_dim={shared_cond_proj.out_dim}")
