...
def get_data(config, extractor, extractor_args):
    ...
    fov = config['data']['fov']
    transforms = Compose([Resize(imsize), ToTensor(), Lambda(to_tensor_and_normalize)])
...
def build_models(config, disc=True):
    ...
    if disc:
        ...
        shared_cond_proj = nerf_model.condition_feature  # DualProjection

        cond_dim = shared_cond_proj.out_dim  # 256
